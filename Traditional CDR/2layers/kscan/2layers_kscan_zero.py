...
# %%
i=18
def run_single_repeat(i, optimized_parameters, primitive, mapper, mol, nonclif_ground_state_energy, noisy_nonclif_ground_state_energy):
    results_linear = []
    results_nonlinear = []
    ...
    for k in range(i):
        ...
        if n_replacements in [1, 17, 18]:
            ...
            result = minimize(cost_function, [1, 0], args=(X_exact, X_noisy))
            ...
            params, _ = curve_fit(nonlinear_model, X_noisy, X_exact, p0=[1, 1, 0])
            a1_opt, a2_opt, a3_opt = params
            y_pred_nonlinear = nonlinear_model(noisy_nonclif_ground_state_energy, a1_opt, a2_opt, a3_opt)
            results_nonlinear.append(y_pred_nonlinear)

    return results_linear, results_nonlinear

results = Parallel(n_jobs=20)(
    delayed(run_single_repeat)(
        i,
        optimized_parameters,
        primitive,
        mapper,
        mol,
        nonclif_ground_state_energy,
        noisy_nonclif_ground_state_energy
    ) for _ in range(20)
)
...
save_to_csv("k scan 2layer zero.csv", {
    "Linear": linear_results,
    "Nonlinear": nonlinear_results,
})

print("Data saved to k scan 2layer zero.csv")
